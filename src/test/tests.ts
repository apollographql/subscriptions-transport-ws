...
describe('Client<->Server Flow', () => {
    ...
    it('should attache the request context to operation params', (done) => {
        ...
        server = createServer(notFoundRequestListener);
        server.listen(SERVER_EXECUTOR_TESTS_PORT);
        const onOperationSpy = sinon.stub();
        onOperationSpy.returns({schema});

        SubscriptionServer.create({
            schema,
            execute,
            onOperation: onOperationSpy,
        }, {
            server,
            path: '/',
        });
        ...
        client.onConnected(() => {
            client.request({
                ...
                getContext: () => operationContext,
                    ...
                    next: (res) => {
                        expect(onOperationSpy.firstCall.args[1].operationContext).to.deep.equal(operationContext);
                    },
                    error: (err) => {
                        assert(false, 'unexpected error from request');
                    },
